			...
			struct ktermios *old)
{
	...
	if ((baud <= 115200) && (port->uartclk == 12000000))
	...
	if (baud <= 115200) {
		...
		if (support_clk12m_baud115200) {
			/*High Speed = 3*/
			serial_port_out(port, MTK_UART_HIGHS, 0x03);
			quot = DIV_ROUND_CLOSEST(port->uartclk, 256 * baud);
			if (quot == 0)
				quot = 1;

			samplecount = DIV_ROUND_CLOSEST(port->uartclk,
							quot * baud);
			if (samplecount != 0) {
				realbaud = port->uartclk/samplecount/quot;
				if ((realbaud > BAUD_ALLOW_MAX(baud)) ||
				    (realbaud < BAUD_ALLOW_MIX(baud))) {
					pr_info("ttyS%d baud%d can't handle\n",
						  serial_index(port), baud);
				}
			} else {
				pr_info("ttyS%d samplecount is 0\n",
					  serial_index(port));
			}
		}
	} else if (baud <= 576000) {
